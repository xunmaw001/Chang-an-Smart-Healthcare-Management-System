...
<template>
    <div class="main-content">
        <el-dialog title="审核" :visible.sync="yuyueYesnoTypesVisible">
            <el-form :model="form">
                <input type="hidden" v-model="form.id">
                <el-form-item label="审核" >
                    <el-select v-model="form.yuyueYesnoTypes" placeholder="请选择审核类型">
                        <el-option label="已到" value="2"></el-option>
                        <el-option label="未到" value="3"></el-option>
                    </el-select>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button @click="yuyueYesnoTypesVisible = false">取 消</el-button>
                <el-button type="primary" @click="yuyueYesnoTypesShenhe">提 交</el-button>
            </div>
        </el-dialog>
        <!-- 条件查询 -->
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '预约状态' : ''">
                        <el-select v-model="searchForm.yuyueYesnoTypes" placeholder="请选择预约状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in yuyueYesnoTypesSelectSearch"
                               v-bind:key="index"
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '医生手机号' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.yishengPhone" placeholder="医生手机号" clearable></el-input>
                    </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('yuyue','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/changanyiliao/upload/yuyueMuBan.xls"
                        >批量导入医生预约数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('yuyue','导入导出')"
                                style="display: inline-block"
                                action="changanyiliao/file/upload"
                                :on-success="yuyueUploadSuccess"
                                :on-error="yuyueUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('yuyue','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "yuyue.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="花费金额">
                        <template slot-scope="scope">
                            {{scope.row.yuyueTruePrice}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="yuyueYesnoTypes"
                                      header-align="center"
                                      label="预约状态">
                        <template slot-scope="scope">
                            {{scope.row.yuyueYesnoValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('yuyue','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <!--<el-button v-if="isAuth('yuyue','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>-->
                            <el-button v-if="isAuth('yuyue','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('yuyue','修改') && scope.row.yuyueYesnoTypes == 1 " type="primary" icon="el-icon-thumb" size="mini" @click="openYesnoTypes(scope.row.id)">审核</el-button>
...
<script>
    ...
    //级联表下拉框搜索条件
    //当前表下拉框搜索条件
              yuyueYesnoTypesSelectSearch : [],
            form:{
                id : null,
                yishengId : null,
                yonghuId : null,
                yuyueTruePrice : null,
                yuyueYesnoTypes : null,
                yuyueTime : null,
                insertTime : null,
                createTime : null,
            },
        ...
        methods: {
            ...
            getDataList() {
                ...
                if (this.searchForm.yishengPhone!= '' && this.searchForm.yishengPhone!= undefined) {
                    params['yishengPhone'] = '%' + this.searchForm.yishengPhone + '%'
                }
                ...
                if (this.searchForm.yuyueYesnoTypes!= '' && this.searchForm.yuyueYesnoTypes!= undefined) {
                    params['yuyueYesnoTypes'] = this.searchForm.yuyueYesnoTypes
                }
                        
                params['yuyueDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=yuyue_yesno_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.yuyueYesnoTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            yuyueUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "yuyue/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            yuyueUploadError(data){
                this.$message.error('上传失败');
            },
            openYesnoTypes(id) {
                let _this = this;
                _this.form.id = id;
                _this.form.yuyueYesnoTypes = "请选择是否到";
                _this.yuyueYesnoTypesVisible = true;
            },

            yuyueYesnoTypesShenhe() {
                let _this = this;
                if(_this.form.yuyueYesnoTypes == "请选择是否到"){
                    alert("请选择是否到");
                    return false;
                }
                this.$http({
                    ...
                    if (data && data.code === 0) {
                        this.$message({
                            ...
                            duration: 1500,
                            onClose: () => {
                                _this.yuyueYesnoTypesVisible = false;
                                _this.search();
                            }
                        });
                    } else {
